...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
        <ul class="navbar-nav">
         ...
         <form method="post" class="text-center">
           <p><h3>¿Desea Eliminar la Reserva?</h3></p>
              <input type="radio" name="check" value="si" >
              <label for="no">Si,eliminar la reserva</label><br>
              <input type="radio"name="check1" value="no">
              <label for="no">No eliminar la reserva</label><br><br>
    ...
    ';
    /*Comprobamos la respuesta seleccionada en el formulario*/
    if (isset($_POST["check"])) {
        /*Consulta para eliminar la reserva seleccionada*/
        $consulta = "DELETE FROM `reservas` WHERE idReservas='" . $id . "'";
        ...
        if ($objconexion->comprobar() > 0) {
            ...
            if (result.isConfirmed) {
                window.top.location="ReservasAdm.php";
            }
        })
          </script>';
        }
    ...
    if (isset($_POST["check1"])) {
        {
            ...
            icon:"error",
            text:"Reserva no eliminada con éxito",
            allowOutsideClick: false,
            allowEscapeKey: false,
            allowEnterKey: false,
            stopKeydownPropagation:false,
