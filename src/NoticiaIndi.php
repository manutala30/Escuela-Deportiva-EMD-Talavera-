...
/*Escogemos el id de la notici*/
$x=$_GET["z"];
/*Consultamos la noticia*/
$consulta="SELECT * FROM noticias n INNER JOIN deporte d on n.IdDeporte= d.IdDeporte where idNoticia=$x";
$objconexion->realizarConsultas($consulta);
        if ($objconexion->comprobarSelect() > 0) {
            /*Extraemos los datos de dicha noticia*/
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <article>
                ...
                <header class="mb-4">
                    <!-- Post title-->
                    <h1 class="fw-bolder mb-1 text-center">'.$fila["Titulo"].'</h1>
                    <a class="badge bg-secondary text-decoration-none link-light" href="#!">'.$fila["Nombre"].'</a>
                   
                </header>
                <!-- Preview image figure-->
                <figure class="mb-5" > <img class="img-fluid rounded"  src="Administrador/noticias/'.$fila["Imagen"].'" /></figure>
                <!-- Post content-->
                <section class="mb-5">
                    <p class="fs-5 mb-4">'.$fila["Desarrollo"].'</p>
              </section>
            </article>
            <!-- Comments section-->
        </div>
